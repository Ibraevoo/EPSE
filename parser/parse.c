...
void parse_and_save(char* filepath, char* folder_name){
        ...
        if (BUFFER[0] == '>')
    {
        ...
        sequence_clear(s);
        id++;
        // New protein header - parse from start to end (end is denoted if we get a space char) - skip 3 char sp| then u get ur id which is till the next | 
        int ptr = 4;
        char accession[32];
        int i =0;
        while(BUFFER[ptr]!='|') {
...
ptr++; // now points to head of description
        ...
        i = 0;
        while(BUFFER[ptr]!='O' && BUFFER[ptr+1]!='S' && BUFFER[ptr+2]!='=') { // get to O of OR 
...
descp[i] = '\0';
// ptr now points to O of OR make it point to the first letter of organism name via skipping R and = 
ptr+=3; // goto R goto = now at beginning
        char org[256];
        i = 0;
        while(BUFFER[ptr]!='O' && BUFFER[ptr] != 'X' && BUFFER[ptr+2]!='=') {
...
parse_and_save("uniprot_sprot.fasta","/folder");
